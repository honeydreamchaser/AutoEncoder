...
t_max = 1000
Feature = 27 
Hidden_feature = 16 #check
Target_y = 13  
Category_Total = [0, 3, 2, 2, 3, 2, 2, 2, 2, 2, 3, 3, 2, 3, 3, 2] #check
Category = Category_Total[Target_y]   #check
...
class RetailerDataset(Dataset):
    def __init__(self, in_feature, target_y, file_id):
        ...
        y_data = []
        self.len = 0
        input_file = open("Retailer" + str(file_id + 1) + "_PPPCA.csv", 'r')
        ...
        for row in input_reader:
            ...
            if y_val > 0:
                ...
                self.len += 1
        input_file.close()
        self.x = torch.from_numpy(np.array(x_data)) #final feature
        self.y = torch.LongTensor(np.array(y_data)) #final target of any particular 
...
def train(epoch): #check
    total_loss = [0 for k in range(7)]
    total_num = [0 for k in range(6)]
    w1 = [torch.zeros(Hidden_feature, Feature) for i in range(5)]  #check
    b1 = [torch.zeros(Hidden_feature) for i in range(5)]  #check 
    w2 = [torch.zeros(Category, Hidden_feature) for i in range(5)]  #check
    b2 = [torch.zeros(Category) for i in range(5)] #check

    for k in range(5):
        optimizer[k].zero_grad() #gradient
        ...
        for retailer_t in range(3): #check
...
def test(epoch, max_record): #check
    model[5].eval()
...
def main():
    print("Before logger")
    # logger = Logger('/Users/vinspatel/Documents/Final_Thesis_Work/log/autoencoder.log')
    print("In the logger")
    for i, data in enumerate(train_loader[0], 0):  # initialize model5 w.grad b.grad
        print("i is here! {} --- {}".format(i,data))
        inputs, labels = data
        y_pred = model[5](inputs)
        loss = criterion[5](y_pred, labels) #check
        loss.backward() #check
        optimizer[5].zero_grad() #check
        break
